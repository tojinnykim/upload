...
# 3. pop을 이용해서 데이터를 삭제 
# 4. 또 다른 리스트를 준비해서 3번에서 삭제한 인덱스,데이터를 저장
# 5. 저장된 리스트에 역순으로 데이터를 뽑아서 insert를 이용해서 복원 

data = []
# num = input( ' 숫자 입력 : ')
# num = int(num) **integer(정수)**의 약자
# data.append(num)
data.append( int(input('숫자입력 : ')) ) # 10
data.append( int(input('숫자입력 : ')) ) # 30
data.append( int(input('숫자입력 : ')) ) # 20
...
print('data= ',data)
#삭제 순서 [10,30,20] [10,20] [20]
#복원순서 [20] [10,20] [10,30,20]

data.insert( removed_list[2][0], removed_list[2][1] ) #a[i][j] → "i번째 리스트의 j번째 값"
print('복원 ',data)
